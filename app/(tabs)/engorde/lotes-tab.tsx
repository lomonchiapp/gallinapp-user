...
export default function LotesTab() {
  ...
  useEffect(() => {
    console.log('🐔 Engorde: Suscribiéndose a lotes, artículos y registros de peso...');
  ...
  }, [suscribirseAEngorde, loadArticulos, subscribeToPesosByTipo, cargarGalpones]);

  // Cargar TODOS los registros de mortalidad de engorde al inicio
  useEffect(() => {
    const cargarRegistrosMortalidad = async () => {
      try {
        console.log('🔄 Cargando TODOS los registros de mortalidad de POLLO_ENGORDE');
        await loadRegistrosPorTipo(TipoAve.POLLO_ENGORDE);
      } catch (error) {
