...
export default function RegistrarPesoScreen() {
  ...
  const handleRegistrarPeso = async () => {
    ...
    try {
      ...
      // Invalidar cache del inventario para actualizar productos disponibles
      invalidateInventarioCache('levantes');
  ...
  const autoLlenarPesos = () => {
    const pesosAleatorios = pesosIndividuales.map(() => {
      if (unidadPeso === WeightUnit.GRAMS) {
        return (Math.random() * 1360 + 1360).toFixed(0); // Pesos entre 1360-2720 gramos (3-6 libras)
        ...
        <View style={styles.formGroup}>
          ...
          <Text style={styles.helperText}>
            {unidadPeso === WeightUnit.GRAMS 
              ? 'Recomendado: 1360-2720 gramos por pollo de levante'
