...
export default function LotesTab() {
  ...
  // Crear una clave única basada en las estadísticas para detectar cambios
  const estadisticasKey = useMemo(() => {
    return Object.keys(estadisticasLotes).map(id => `${id}:${estadisticasLotes[id]?.huevos || 0}:${estadisticasLotes[id]?.muertes || 0}`).join('|');
  }, [estadisticasLotes]);
  ...
  useEffect(() => {
    ...
    const calcularCostos = async () => {
      ...
      timeoutId = setTimeout(async () => {
        ...
        const nuevosCostos: {[loteId: string]: {costoTotal: number, costoPorHuevo: number, costoUnitario: number, isLoading: boolean}} = {};
        const nuevosGastosPorLote: {[loteId: string]: {fecha: Date, total: number}[]} = {};

        // Calcular costo para cada lote en paralelo
        const promesas = lotes.map(async (lote) => {
          if (!isMounted) return;
          ...
          try {
            ...
            nuevosGastosPorLote[lote.id] = gastosLote.map(g => ({
            ...
            if (isMounted) {
              nuevosCostos[lote.id] = { costoTotal: 0, costoPorHuevo: 0, costoUnitario: 0, isLoading: false };
            }
          }
        });
        ...
        if (isMounted) {
          ...
          setCostosPorHuevo(prev => ({ ...prev, ...nuevosCostos }));
          setGastosPorLote(prev => ({ ...prev, ...nuevosGastosPorLote }));
        }
      }, 500); // Debounce de 500ms para evitar cálculos excesivos
    };
  ...
  }, [lotes.length, registrosMortalidad.length, estadisticasKey]);
  ...
  useEffect(() => {
    ...
    // Marcar que se ha intentado cargar al menos una vez
    const timeoutId = setTimeout(() => {
      setHasLoadedOnce(true);
    }, 2000); // Después de 2 segundos, asumir que la carga inicial terminó
  ...
  useEffect(() => {
    if (lotes.length > 0) {
      setHasLoadedOnce(true); // Marcar que se han cargado lotes
      // Usar un timeout para evitar cargas múltiples rápidas
      const timeoutId = setTimeout(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [lotes.length]); // Solo cuando cambia la cantidad de lotes, no el array completo

  // Cargar información de tracking de recolección de huevos usando datos del store
  useEffect(() => {
    if (lotes && lotes.length > 0 && registrosHuevos) {
      // Debounce para evitar cálculos excesivos
      const timeoutId = setTimeout(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [lotes.length, registrosHuevos.length]); // Solo cuando cambian las longitudes
  ...
  const handleNuevoLote = () => {
    router.push('/(tabs)/mi-granja/ponedoras/nuevo-lote');
  };
  ...
  const handleVerLote = (loteId: string) => {
    router.push(`/(tabs)/mi-granja/ponedoras/detalles/${loteId}` as any);
  };
    ...
    const acciones: AccionLote[] = [
      {
        ...
        icon: 'skull-outline',
        onPress: () => router.push(`/(tabs)/mi-granja/ponedoras/registrar-muerte?loteId=${lote.id}`),
        variant: 'default',
      },
      {
        ...
        icon: 'egg-outline',
        onPress: () => router.push(`/(tabs)/mi-granja/ponedoras/registrar-produccion?loteId=${lote.id}`),
        variant: 'primary',
      },
      {
        id: 'registrar-gasto',
  ...
  // Estado de carga inicial - solo mostrar loading si realmente está cargando y no ha cargado antes
  const isInitialLoading = isLoading && !hasLoadedOnce && lotes.length === 0;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <Button
          title="Dashboard"
          onPress={() => router.push('/(tabs)/mi-granja/ponedoras/dashboard-comparativo')}
          variant="outline"
          size="small"
        ...
        <View style={styles.headerActions}>
          <Button 
            title="Historial RH" 
            onPress={() => router.push('/(tabs)/mi-granja/ponedoras/historial-registros-huevos')} 
            size="small"
            variant="outline"
      ...
      {isLoading ? (
        ...
        <View style={styles.emptyState}>
          <PonedoraIcon width={64} height={64} fill={colors.lightGray} />
                ...
                {estaExpandido && (
                  <View style={styles.loteExpandedContent}>
                    <View style={styles.loteStats}>
                      ...
                      <TouchableOpacity 
                        ...
                        onPress={() => {
                          ...
                          router.push({
                            pathname: '/(tabs)/mi-granja/ponedoras/registros-huevos',
                    ...
                    <View style={styles.loteActions}>
                      <Button 
                        title="Editar" 
                        onPress={() => router.push(`/(tabs)/mi-granja/ponedoras/editar-lote?loteId=${lote.id}` as any)} 
                        variant="outline"
                        size="small"
...
const styles = StyleSheet.create({
  ...
  contentContainer: {
    ...
    paddingBottom: 90, // Espacio extra para el botón flotante
  },
  header: {
    flexDirection: 'row',
