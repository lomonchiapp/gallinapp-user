/**
 * Pantalla para registrar mortalidad en lotes de ponedoras
 */
...
export default function RegistrarMuerteScreen() {
  ...
  const { lotes, cargarLote, isLoading: loteLoading } = usePonedorasStore();
  ...
  const handleRegistrarMuerte = async () => {
    ...
    if (lote && cantidadNum > lote.numeroAves) {
      Alert.alert('Error', `La cantidad de aves muertas no puede ser mayor que la cantidad actual de aves (${lote.numeroAves})`);
      return;
    }
      ...
      <Card style={styles.card}>
        ...
        {loteLoading ? (
          ...
          <View style={styles.loteInfoContainer}>
            ...
            <Text style={styles.loteInfo}>
              <Text style={styles.loteInfoLabel}>Aves actuales:</Text> {lote.numeroAves}
            </Text>
            <Text style={styles.loteInfo}>
