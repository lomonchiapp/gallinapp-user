/**
 * Pantalla para ver, editar y eliminar registros de producción de huevos
 */
...
export default function RegistrosHuevosScreen() {
  const params = useLocalSearchParams<{ loteId: string | string[] }>();
  // Manejar cuando loteId viene como array o string
  const loteId = Array.isArray(params.loteId) ? params.loteId[0] : params.loteId;
  
  const [registros, setRegistros] = useState<HuevoRegistro[]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [modalEditarVisible, setModalEditarVisible] = useState(false);
  const [registroEditando, setRegistroEditando] = useState<HuevoRegistro | null>(null);
  const [cantidadEditando, setCantidadEditando] = useState('');
  const [fechaEditando, setFechaEditando] = useState('');
  const [guardando, setGuardando] = useState(false);

  useEffect(() => {
    console.log('RegistrosHuevosScreen - loteId recibido:', loteId);
    if (loteId) {
      cargarRegistros();
    } else {
      console.warn('No se recibió loteId en los parámetros');
      showErrorAlert('Error', 'No se pudo identificar el lote');
    }
  }, [loteId]);

  const cargarRegistros = async () => {
    if (!loteId) {
      console.warn('cargarRegistros: No hay loteId');
      return;
    }
    
    try {
      setLoading(true);
      const datos = await obtenerRegistrosHuevos(loteId);
      setRegistros(datos);
    } catch (error: any) {
      showErrorAlert('Error', error.message || 'Error al cargar registros');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const abrirModalEditar = (registro: HuevoRegistro) => {
  ...
  const handleGuardarEdicion = async () => {
    ...
    if (isNaN(cantidadNum) || cantidadNum < 0) {
      showErrorAlert('Error', 'Por favor ingrese una cantidad válida');
      return;
    }

    // Validar fecha
    if (!fechaEditando) {
      showErrorAlert('Error', 'Por favor seleccione una fecha');
      return;
    }
    ...
    try {
      ...
      showSuccessAlert('Éxito', 'Registro actualizado correctamente', () => {
        cerrarModalEditar();
        cargarRegistros();
      });
    } catch (error: any) {
      showErrorAlert('Error', error.message || 'Error al actualizar registro');
    } finally {
      setGuardando(false);
    }
  };

  const handleEliminar = (registro: HuevoRegistro) => {
    showConfirmationAlert(
      'Confirmar eliminación',
      `¿Está seguro de eliminar el registro del ${formatDate(registro.fecha)} con ${registro.cantidad} huevos?`,
      async () => {
        try {
          await eliminarRegistroHuevos(registro.id);
          showSuccessAlert('Éxito', 'Registro eliminado correctamente', () => cargarRegistros());
        } catch (error: any) {
          showErrorAlert('Error', error.message || 'Error al eliminar registro');
        }
      },
      undefined,
      'Eliminar',
  ...
  const totalHuevos = registros.reduce((sum, registro) => sum + registro.cantidad, 0);
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Registros de Producción</Text>
        </View>
        ...
        <Card style={styles.resumenCard}>
          <View style={styles.resumenContent}>
            ...
            <View style={styles.resumenText}>
              ...
              <Text style={styles.resumenValue}>{registros.length}</Text>
            </View>
          </View>
        </Card>
